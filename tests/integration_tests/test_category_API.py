async def test_add_category(client):
    response = await client.post("/categories/", json={"name": "name"})

    assert response.status_code == 200
    assert response.json()["name"] == "name"
    assert "id" in response.json()


async def test_get_categories(client):
    await client.post("/categories/", json={"name": "name"})
    response = await client.get("/categories/")

    assert response.status_code == 200
    assert response.json() == [{"name": "name", "id": 1}]


async def test_get_category_by_id(client):
    await client.post("/categories/", json={"name": "name"})
    response = await client.get("/categories/1")

    assert response.status_code == 200
    assert response.json() == {"name": "name", "id": 1}


async def test_delete_category(client):
    await client.post("/categories/", json={"name": "name"})
    response = await client.delete("/categories/1")

    assert response.status_code == 200
    assert response.json() == {"name": "name", "id": 1}


async def test_get_products_by_category_id(client):
    await client.post("/categories/", json={"name": "name"})
    await client.post("/categories/", json={"name": "name2"})
    await client.post("/products/", json={"name": "name1", "category_id": 1})
    await client.post("/products/", json={"name": "name2", "category_id": 1})
    await client.post("/products/", json={"name": "name3", "category_id": 2})
    await client.post("/products/", json={"name": "name4", "category_id": 1})

    products = [
        {"name": "name1", "category_id": 1, "id": 1},
        {"name": "name2", "category_id": 1, "id": 2},
        {"name": "name3", "category_id": 2, "id": 3},
        {"name": "name4", "category_id": 1, "id": 4},
    ]

    response = await client.get("categories/1/products")

    assert response.status_code == 200
    assert response.json()[0] == products[0]


async def test_delete_category_raises_not_found_error(client):
    response = await client.delete("/categories/9999")

    assert response.status_code == 404


async def test_get_category_by_id_raises_not_found_error(client):
    response = await client.get("/categories/999")

    assert response.status_code == 404
